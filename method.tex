...
polynomial fit, $P(X, [\ion{Fe}/\ion{H}])$ is added to} \eref{eq:irfm} \hl{so the calibration
finally reads:}
\begin{align}
...
Asteroseismology is the study of stellar pulsations. \hl{For main sequence stars (which will be the
...
\sref{sec:parameters}) \hl{is that it allows for the analysis of lower resolution and with a higher
rotational velocity, $v\sin i$.}


\section{The EW method: \code{FASMA}}
...
\st{When a line is measurement automatically it is in this thesis done with} \code{ARES}
\hl{Throughout this thesis line EWs are automatically measured with} \code{ARES}
...
\begin{itemize}
  ...
  \item Smoothing applied with a \code{boxcar} filter before measuring the EWs. \hl{This is only for
        automatic line identification.}
\end{itemize}
An in-depth description of these options can be found in
\citet{Sousa2007,Sousa2015a}.

\st{Sometimes} \hl{Rarely} \code{ARES} crash when measuring an absorption line. The reason is not
...
sharp peaks in the spectra, etc.} To deal with this problem, \code{FASMA} finds the last line which
\code{ARES} tried to measure in the log file. This line is temporarily removed from the line list
and \code{ARES} is restarted. The line list used for deriving parameters consists of numerous iron
...
\fref{fig:vtRelation} \hl{for a sample of 583 stars analysed with} \code{FASMA}. \code{FASMA} use
...
below \SI{5200}{K}. Therefore, if the option \code{teffrange} is on (which is highly recommended),
