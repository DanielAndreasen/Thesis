...
stars (\sref{sec:planet_host_stars}), which is the main focus \st{on} \hl{of} this thesis. While learning about
host stars, and stars in general, the results have wide-spread applications, where some will briefly
be discussed in the end of this chapter (\sref{sec:stars_application}) before \st{an introduction on
what} \hl{a general overview of} this thesis \st{will consists of} (\sref{sec:this_thesis}).
...
combined. \hl{The success of the of different methods can be seen in} \fref{fig:detectionTypes},
...
\fref{fig:transitMethod} \hl{for K2-29 b}.
...
hence $e=0$. The sinusoidal motion of the star can be seen in \fref{fig:rvmethod} where both the
