...
(\ion{Fe}, \ion{Si}, \ion{Mg}) \hl{are key to reduce the degeneracy in interior structure models and
mantle composition.}
...
\fref{fig:detectionTypes}. \st{W}Here all the detected planets (with measured masses) are shown
...
star. In} \fref{fig:transitAll} \hl{all detected exoplanets with the transit method is shown. In
...
of the star can be seen in \fref{fig:rvmethod} where both the time series and the phase curve is
presented for the exoplanet, K2-29 b. The data used is from the same source as the lightcurve above
in \sref{sec:transitMethod}.
...
illustrated in} \fref{fig:RMeffect}.
...
to the RV method described above in \sref{sec:rvmethod}. \st{Unlike the RV method, this technique
...
of the host star (see \eref{eq:rv}).
...
surface gravity. \hl{The data for} \fref{fig:hostDistribution} \hl{is obtained from
...
\tref{tab:standardErrors}. The parameters are from SWEET-Cat, and for the FGK stars only parameters
...
\hl{The detailed characteristics of M dwarfs are currently a problem,} however, with the advance of
...
all be used in \cref{cha:results}. First by obtaining a NIR line list containing \ion{Fe}{I} and
