...
parameters of a star. This is discussed in \sref{sec:EPcut}, and last the analysis of a range of
...
for employing the method of deriving the stellar atmospheric parameters described here. This line
list is composed by neutral and ionized iron lines in the NIR. This has already been mentioned in
\cref{cha:theory}. Here the process will be explained in greater detail below for the two versions
presented in \citet{Andreasen2016} and \citet{Andreasen2017b}, respectively.

The two versions of the line lists are presented in this section together, rather than when the
results from this thesis were obtained. The first version of the line list was presented with the
analysis of HD 20010, while the second version of the line list was presented in 10 Leo and
Arcturus. However, since the line list is a tool to reach the goal, the stellar atmospheric
parameters, they are both presented together before being applied below.
...
the YJHK bands.} were downloaded from the VALD database \citep{VALD1,VALD2}. This only includes
...
of the line, the EW of the line, and the Gaussian coefficients of the line. In the latest version of
\code{ARES}, the error of the EW is also provided \citep{Sousa2015a}.
...
lines (including the iron line(s)) were plotted on top of the solar spectrum. An iron line was
...
\ion{Fe}{II} lines were reduced to a mere 593 and 22, respectively.\unfinished{Make some plots for
...
$[\ion{M}/\ion{H}]=0.00$ by definition. This was done for all spectral windows for three different
iron abundances $[\ion{Fe}/\ion{H}]=\{-0.20;\,0.00;\,0.20\}$ in order to identify iron lines and
their impact on the synthetic spectra. \unfinished{Write about the \code{synth} driver an options
used...} Before creating a synthetic spectrum all elements which are more than singly ionised were
removed since \code{MOOG} does not allow these. An example of the three synthetic spectra can be
seen in \fref{fig:synthetic_investigation}. Here the neutral iron line at \SI{15550.439}{\angstrom}
was investigated. The three coloured curves are synthetic spectra with the three different
$[\ion{Fe}/\ion{H}]$. The upper plot shows the result with the full VALD3 line list, while in the
lower plot the iron line were removed from the line list. In this case it is clear that the iron
line is the cause of the absorption line. The grey curve is the solar atlas for reference. Note that
it is not the purpose to match a synthetic spectrum to the solar atlas.
...
\unfinished{Make a plot with line abundances before calibration to clearly show why it is done.}
...
(\code{IRAF}). The reason not to choose \code{ARES} for this task was to visually confirm the
...
of the line list. Whenever there were more than one EW measurement of a line, due to overlap in
different spectra, the average was used for the final EW. The stellar atmospheric parameters were
...
manually measure the EWs for one set (summer). Since this is a test of the line list (rather than a
study of Arcturus) and the fact that the EWs are very similar, the parameters are only derived from
the summer set with the EWs measured by \code{ARES}. Parameters were derived with and without $\log
g$ set to a fixed value (\SI{1.60}{dex}, the average literature value adopted). The derivation of
the parameters followed the same procedure as described above for HD 20010 using \code{FASMA}. Again
outliers in abundance were removed one at the time until there were no more outliers. The final
...
\unfinished{Make some histograms of $[\ion{Fe}/\ion{H}]$}
...
measured EWs as they will be blended and deeper than expected.
\unfinished{Make a plot with EWs for lines in common and look for any offsets.}
