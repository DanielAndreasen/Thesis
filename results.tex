...
\MOOG does not allow these. An example of this can be seen in \fref{fig:synthetic_investigation}.

\begin{figure}[htpb!]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/synthetic_investigation.pdf}
    \caption{Synthetic investigation}
...
7m\angstrom. This means that the two data sets are very similar, thus we decided to only manually
measure the EWs for one set (summer). We did, however, measure a few lines from the winter data set
to verify the agreement. Since the EWs are very similar we chose to only derive parameters of the
summer set with EWs measured with \ARES. Parameters were derived with and without $\log g$ set to a
fixed value (1.60\,dex, the average literature value adopted). The derivation of the parameters
