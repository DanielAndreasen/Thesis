...
some reliable values instead}, and this was a derived parameter from a empirical relation. $\log g$
...
for each of the 12 synthetic spectra. These line list were used to derive parameters with two
different model atmospheres, ATLAS9 \citep{Kurucz1993} and MARCS \citep{Gustafson2008}, with \FASMA.
This was done twice, with all parameters derived from \FASMA (\fref{fig:phoenix_free}), and again
with $\log g$ fixed at \SI{4.5}{dex} and $\xi_\mathrm{micro}$ fixed which means it is changed in
each iteration (see \sref{sec:parameters} for details). The latter case with fixed parameters is
shown in \fref{fig:phoenix_fix}.
...
For the results with $\log g$ and $[\ion{Fe}/\ion{H}]$ fixed at the correct parameters during the
...
this limit the $T_\mathrm{eff}$ are slightly underestimated with a hint of worse results further
from this limit. The reason for this can partly be found in the measured \ion{Fe}{II} lines. As
mentioned before, these are used to determine $\log g$. There have been measured between 1 and 3
\ion{Fe}{II} lines, generally fewer the lower $T_\mathrm{eff}$ is, out of 5 possible measurements.
The mean derived values for the surface gravity are $\log g=(1.11\pm1.63)\,\si{dex}$ and
$\log g=(1.38\pm1.45)\,\si{dex}$ for ATLAS9 and MARCS model atmospheres, respectively. Similarly for
the metallicity the following mean values are $[\ion{Fe}/\ion{H}]=\SI{-0.73(37)}{dex}$ and
