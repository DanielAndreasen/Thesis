...
It is time to apply the theory and methodology on some real data. \hl{The tools presented above will
be used on NIR spectra of three stars. The goal is to derive parameters in the NIR. The reason for
...
characteristics can be found in \tref{tab:stars}. \hl{A plot of the spectra of the four stars
analysed in this chapter is shown in} \fref{fig:NIRspectra}. More details are provided in the
individual sections below for each star. \st{An update to SWEET-Cat, a catalogue which provides
homogeneous stellar parameters of planet hosts, was provided in}. Last is the analysis of a range of
...
were measured using \code{ARES} due to the large amount of lines and to be as consistent as possible
in the measurements. Since \code{ARES} expect a 1D spectrum with equidistant wavelength
step\footnote{\hl{This is changed in the newer version of} \code{ARES} \citep{Sousa2015a}.}, the
...
of the line, and the EW of the line \st{of the line}. In the latest version of \code{ARES}, the
...
are more than singly ionised were removed since \code{MOOG} does not allow these. An example of the
...
abundance for all iron lines are 7.47 when using the solar atmosphere model mentioned above. \st{As
mentioned in} There is an anti-correlation between the abundance of a line and $\log \mathit{gf}$.
This means a simple bisector minimization can be used to locate the $\log\mathit{gf}$ that gives the
desired abundance. This was done for all the iron lines at this stage.
...
radiative code, here \code{MOOG}, and even the settings used in the radiative transfer code, i.e.
the specific physics used. In the results listed below the setups are identical, except for the work
done with synthetic spectra (see \sref{sec:synthetic_spectra}). There both ATLAS9 and MARCS model
...
\sref{sec:HD20010_first}), however the derived parameters showed poor results when compared to the
...
As \st{will be} \hl{was} seen in \sref{sec:HD20010_first} the line list presented above was used to
...
(see \sref{sec:HD20010_first} for details on this). Additionally, the derived metallicity was
...
\fref{fig:linelist_comparison} where the \st{EW measurements from the first version is shown against
...
\SI{2.1+-11.1}{m\angstrom}. \hl{The large deviation are due to line blending.} There are some
...
\begin{figure}[htpb!]
    ...
    \caption{Comparison of the EW from the first version of the line list (\hl{measured with}
             \code{ARES}), EW$_1$, and the second version, EW$_2$ (\hl{measured with} \code{IRAF}).
