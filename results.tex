...
found in \tref{tab:stars}. A plot of the spectra of the four stars analysed in this chapter is shown
in \fref{fig:NIRspectra}. More details are provided in the individual sections below for each star.
...
described above in \sref{sec:parameters}. However, before diving into the results of the analysis it
...
measured using \code{ARES} due to the large amount of lines and to be as consistent as possible in
the measurements. Since \code{ARES} expect a 1D spectrum with equidistant wavelength
step\footnote{This is changed in the newer version of \code{ARES} \citep{Sousa2015a}.}, the solar
...
of the line, and the EW of the line. In the latest version of \code{ARES}, the error of the EW is
...
ionised were removed since \code{MOOG} does not allow these. An example of the three synthetic
spectra can be seen in \fref{fig:synthetic_investigation}. Here the neutral iron line at
...
As was seen in \sref{sec:HD20010_first} the line list presented above was used to derive atmospheric
...
\fref{fig:linelist_comparison} where the difference in the EW measurements are shown against the EWs
...
\begin{figure}[htpb!]
    ...
    \caption{Comparison of the EW from the first version of the line list (measured with
             \code{ARES}), EW$_1$, and the second version, EW$_2$ (measured with \code{IRAF}).
...
the few \ion{Fe}{II} (three lines) measured. However, the $\log g$ is not far from the literature
