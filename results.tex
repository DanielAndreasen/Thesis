...
\unfinished{Make a histogram with line abundances before calibration to clearly show why it is
done.} These lines were collected into a single line list in the format required by MOOG
\citep{Sneden1973} and the line abundance were measured by all lines using the solar atmosphere
model described above. If the derived abundance for a single line differs by more than \SI{1.0}{dex}
from the solar iron abundance, the line was discarded.
...
expected. \unfinished{Make a plot with EWs for lines in common and look for any offsets.}
