...
reported. We note that this parameter is not reported in the PASTEL database\improve{Then find some
reliable values instead}, and this was a derived parameter from a empirical relation.
$\log g$ was derived with large errors which is a result from the few \ion{Fe}{II} (three lines)
measured. However, the $\log g$ is not far from the literature value, only \SI{0.34}{dex} lower,
which is well within the large errors reported.
