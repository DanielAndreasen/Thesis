...
\fref{fig:elevel} for hydrogen. This means that the difference in energy levels
gets smaller, or in other words it gets redder. Therefore, the NIR absorption
lines are, in average, produced at higher excitation potentials, which means in
...
$\log \mathit{gf}$, so a simple bisector minimisation can be used to find the
...
abundance for each line and imposing excitation and ionisation balance.


\subsection{Excitation and ionisation balance}
...
\ion{Fe}{I} and \ion{Fe}{II}. This scalar function can be minimised using
standard minimisation procedures as the simplex downhill among others. However,
...
for $\log g$. In the end it is a vector function being minimised which are more
difficult, however we are not minimising this using standard mathematical
methods, but rather using the physical knowledge. This minimisation is useless
...
minimisation is depicted in \fref{fig:minimization}. This minimisation is
...
all iteratively, meaning that after removing the outliers the minimisation is
...
of the minimisation if the $\xi_\mathrm{micro}$ value is close to either
...
indicators to reach convergence, thus making the minimisation less sensitive to
...
\begin{figure}[htpb!]
    ...
    \caption{Overview of the minimisation for FASMA. Credit: \citet{Andreasen2017a}.}
